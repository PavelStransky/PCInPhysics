...
\begin{document}
	...
	\label{sec:odstavce}
		...
		\LaTeX{} automaticky dělí slova.
		\LaTeX{} automaticky dělí slova.
		Pokud se mu to nepodaří, napovíme: nej\-ku\-la\-ťou\-lin\-ka\-těj\-ší.
	...
	\section{Doplňující komentáře (2025)}
		Při psaní českých čísel s desetinnou čárkou \LaTeX{} přidává nevhodně mezeru za čárku.
		Toho se lze zbavit buď uzavřením čárky do složených závorek, nebo vymazáním čárky pomocí příkazu \verb+\!+.

		Jednotky se píší vzpřímeným písmem (příkaz \verb+\mathrm+), aby se odlišily od proměn\-ných, a před jednotku se 
		přidává mezera. Správně tedy $p=1,\!24\,\mathrm{hPa}$.

		Vícepísmenné veličiny a označení veličin v indexech se obvykle píší vzpříměně, 
		aby se graficky odlišilo od součinu veličin nebo od indexů.
		Reynoldsovo číslo se tedy zapíše jako $\mathrm{Re}$.
		Podobně maximální tlak $p_{\mathrm{max}}$.
	
		Při psaní textu (a nejen jeho) je znakem profesionality konzistence, 
		což obnáší dodržovat stejný styl formátování, zápisu veličin a strukturování textu v celém dokumentu.
		\LaTeX{} k tomu významně pomáhá.
...
Expression:
$$ \frac{4 \pi^{2} L a}{T^{2} \cos{\left(t \right)}} $$

Error propagation formula:
$$ \Delta f = \sqrt{\frac{16 \pi^{4} L^{2} \Delta a^{2}}{T^{4} \cos^{2}{\left(t \right)}} + \frac{16 \pi^{4} L^{2} \Delta t^{2} a^{2} \sin^{2}{\left(t \right)}}{T^{4} \cos^{4}{\left(t \right)}} + \frac{64 \pi^{4} L^{2} \Delta T^{2} a^{2}}{T^{6} \cos^{2}{\left(t \right)}} + \frac{16 \pi^{4} \Delta L^{2} a^{2}}{T^{4} \cos^{2}{\left(t \right)}}} $$

Final result:
$$ f = 24.297 \pm 0.2 $$

\begin{equation}
g = \frac{4 \pi^{2} L a}{T^{2} \cos{\left(\theta \right)}}
\end{equation}

\begin{equation}\begin{aligned}
\Delta g &= \sqrt{\left( \frac{\partial g}{\partial L} \Delta L \right)^2 + \left( \frac{\partial g}{\partial T} \Delta T \right)^2 + \left( \frac{\partial g}{\partial \theta} \Delta \theta \right)^2} \\
&= \sqrt{\left( \frac{4 \pi^{2} a}{T^{2} \cos{\left(\theta \right)}} \Delta L \right)^2 + \left( - \frac{8 \pi^{2} L a}{T^{3} \cos{\left(\theta \right)}} \Delta T \right)^2 + \left( \frac{4 \pi^{2} L a \sin{\left(\theta \right)}}{T^{2} \cos^{2}{\left(\theta \right)}} \Delta \theta \right)^2} \\
&\approx \sqrt{5.9 \times 10^{-4} + 1.34 \times 10^{-2} + 5.99 \times 10^{-2}} \;\mathrm{m/s^2}
\end{aligned}\end{equation}

\begin{equation}\begin{aligned}
g &= (2.429 \pm 0.027) \times 10\;\mathrm{m/s^2}\\
\left(\Delta g\right)_{rel} &= 1.12\%
\end{aligned}\end{equation}
