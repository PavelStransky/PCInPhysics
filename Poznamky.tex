...
\subsubsection{Stav souborů v repozitáři}
    \begin{itemize}
        \item \emph{Nesledovaný (untracked):} Systém Git historii tohoto souboru neuchovává.
            Každý nové vytvořený soubor je v tomto stavu.

        \item \emph{Připravený k zapsání (staged):} Soubor je systémem Git sledován (verzován).
        ...
        \item \emph{Změněný (modified):} Soubor je systémem Git sledován (verzován).
            Od předchozího zapsání v něm došlo ke změnám, avšak soubor ještě nebyl označen k zapsání.
            Přehled změn v souborech oproti naposledy uložené verzi dá příkaz
            \begin{lstlisting}
        git diff\end{lstlisting}
        
        \item \emph{Zapsaný (committed):} Soubor je systémem Git sledován(verzován) a je zapsaný v repozitáři.
...
\subsubsubsection{Změny v souboru}
    ...
    \begin{lstlisting}[style=TinyPython]
        ...
        def solve(model, step=step_euler_1, dt=0.1, maxt=10, initial_condition=1):
        ...
        ys, ts = solve(relaxation)\end{lstlisting}
    Po spuštění kód vyřeší diferenciální rovnici relaxace~\eqref{eq:Exp} a v proměnných \code{ys} a \code{ts} vrátí seznam $y$-ových hodnot a odpovídajících časových okamžiků $t$.
    ...
    \begin{center}\includegraphics[width=0.6\linewidth]{GitStatusCommit2.png}\end{center}
...
\subsubsubsection{Přidání dalšího souboru do projektu}
    ...
    \begin{lstlisting}[style=TinyPython]
        ...
        ys, ts = ode.solve(relaxation)\end{lstlisting}
...
\label{sec:gitignore}
    ...
    {\color{ForestGreen}
    \begin{task}
        Podívejte se do souboru \code{\href{https://github.com/PavelStransky/PCInPhysics2021/blob/main/.gitignore}{.gitignore}} v repozitáři k těmto zápiskům.
...
\label{sec:numpy}
    ...
    Základy práce s řadami jsou vysvětleny přímo v souboru \file{\href{https://github.com/PavelStransky/PCInPhysics2021/blob/main/python/arrays.py}{arrays.py}} v repozitáři.
...
\label{task:ODR1}
    ...
    Vyřešte numericky diferenciální relaxační rovnici
...
\label{task:KumulovanaChyba}
    ...
    \begin{equation}\label{eq:ExpChyba}
...
\label{sec:ODRn}
    ...
    \begin{example}
        ...
        \begin{equation}
            Ma=F(y),
        \end{equation}
        kde $M$ je hmotnost pohybujícího se tělesa, $x=x(t)$ jeho poloha a $a=a(t)=\d^{2}x/\d t^{2}$ zrychlení převedeme na~dvě diferenciální rovnice prvního řádu zavedením rychlosti $v=v(t)=\d x/\d t$:
        \begin{equation}
            \derivative{}{t}\makematrix{y \\ v}=\makematrix{v \\ \frac{1}{M}F(y)},
        \end{equation}
        tj. vektor funkce pravých stran podle rovnice~\eqref{eq:ODR} je
        \begin{equation}
            \vector{f}(\vector{y},t)=\makematrix{v \\ \frac{1}{M}F(y)}
        \end{equation}
        kde $\vector{y}\equiv(x,v)$.
    \end{example}
...
\subsection{Symplektické algoritmy}
    ...
    \begin{align}
        x_{i+1}&=y_{i}+v_{i}\Delta t+\frac{1}{2}a_{i}\Delta t^{2},\nonumber\\
...
\begin{task}
    ...
    \begin{equation}\label{eq:HO}
        \derivative[2]{x}{t}=-x
    \end{equation}
    s počátečními podmínkami $y_{0}=0$, $y'_{0}\equiv v_{0}=1$ a porovnejte řešení různými metodami s analytickým řešením $x(t)=\sin t$.
...
\begin{task}
    ...
    Energie harmonického oscilátoru~\eqref{eq:HO} je dána vzorcem
